...
async function loadProducts(){
  ...
  }).join('') || `<tr><td colspan="${isAdmin ? 11 : 9}" class="empty">Belum ada produk</td></tr>`;
...
async function loadStockPage(){
  ...
  <option 
    ...
    ${p.name} (Office: ${p.warehouse_stock}) - ${p.category_name}
...
`).join('');
  ...
  transferSel.innerHTML = products.map(p => `<option value="${p.id}">${p.name} - ${p.category_name}</option>`).join('');
...
// DETEKSI PAGE OPNAME
const opnamePage = document.getElementById('page-opname');
const isOpnameActive = opnamePage && opnamePage.classList.contains('active');
...
// default: stock masuk/keluar
findProductByBarcode(barcode);

      try{

        const products = await api('/products?q=' + barcode);
        ...
        if(products.length){
          ...
          toast('Produk ditemukan: ' + products[0].name);
...
function onTransferProductChange(){
  const id = document.getElementById('transferProductSelect').value;
  ...
  if(!product){
    info.innerHTML = 'Pilih produk terlebih dahulu';
    return;
  }

  info.innerHTML = `
    📦 Office: <b>${product.warehouse_stock}</b> pcs
    &nbsp; | &nbsp;
    🛒 Mess: <b>${product.display_stock}</b> pcs
  `;
}
...
async function loadOpnamePage(){
  ...
  if(scanInput && !scanInput.dataset.listenerAdded){
    ...
    scanInput.addEventListener('input', () => {
      ...
      // Debounce 250ms untuk ketik manual
      clearTimeout(debounceTimer);
      debounceTimer = setTimeout(() => showOpnameDropdown(val), 250);
    });
    ...
    scanInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        ...
        const val = scanInput.value.trim();
        if (!val) return;
        // Scanner fisik kirim Enter setelah barcode lengkap → cari exact match
        const exact = opnameProducts.find(p =>
          String(p.barcode || '').trim() === val
        );
        if (exact) {
          pilihProdukOpname(exact);
        } else {
          // Tidak exact → tampilkan dropdown supaya bisa pilih manual
